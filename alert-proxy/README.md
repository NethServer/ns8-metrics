...
For alert names handled by the generic fallback, a non-empty string
`module_id` is inserted immediately before the existing `node` suffix.
...
Explicitly mapped alert names retain their legacy identifiers even when they
carry `module_id`. Generic alerts without `module_id` also remain unchanged:
...
This preserves existing portal identifiers while preventing generic
