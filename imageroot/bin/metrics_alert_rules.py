...
class AlertRuleSource:
    ...
    @property
    def filename(self):
        return generated_rule_filename(self.module_id, self.rule_set_name)
...
def _normalize_document(document, source):
    ...
    return {
        "groups": [
            {
                "name": f"ns8:{source.module_id}:{source.rule_set_name}",
                "rules": [document],
            }
        ]
    }, True
...
def _rewrite_group_names(document, source, single_rule):
    ...
    for group in document["groups"]:
        ...
        group["name"] = (
            f"ns8:{source.module_id}:{source.rule_set_name}:{local_name}"
        )
...
def _warn_matcher_override(source, warning):
    warning(
        ...
        f"module_id matcher; using module ID {source.module_id!r}"
    )
...
def rewrite_promql_expression(expression, source, promtool_runner, warning=None):
    ...
    placeholder_label = PLACEHOLDER_LABEL_BASE
    while placeholder_label in formatted:
        placeholder_label += "_"
...
def _warn_label_override(source, location, old_value, warning):
    warning(
        ...
        f"{old_value!r}; using module ID {source.module_id!r}"
    )
...
def transform_alert_rule(source, promtool_runner=None, warning=None):
    ...
    try:
        generated_rule_filename(source.module_id, source.rule_set_name)
